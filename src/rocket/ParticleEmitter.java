...
public class ParticleEmitter extends Entity {
    ...
    public ParticleEmitter(double groundY, Color[] particleColorRange, double xOffset, double yOffset) {
        super(0, 0, Color.RED, xOffset, yOffset);
        this.groundY = groundY;
        this.particleColorRange = particleColorRange;
    }

    public ParticleEmitter(double width, double height, double groundY, Color[] particleColorRange, double xOffset, double yOffset, double particleLaunchAngle, Color color) {
    ...
    private Color generateRandomParticleColor() {
        if (getParticleColorRange().length == 1 || getParticleColorRange()[0].equals(getParticleColorRange()[1])) {
            ...
            double particleG = lowerR + (Math.random() * (upperG - lowerG));
            double particleB = lowerR + (Math.random() * (upperB - lowerB));
    ...
    private void emitParticle() {
        double minAngle = getDirection() - getParticleSpreadAngle() + getParticleLaunchAngle();

        double maxAngle = getDirection() + getParticleSpreadAngle() + getParticleLaunchAngle();

        getParticles().add(new Particle(getX(), getY() + getHeight() / 2, getParticleRadius(), generateRandomParticleColor(), minAngle, maxAngle, getVelocity().getMagnitude(), getInitialParticleSpeed(), groundY));
    }
    ...
    private void cleanParticles() {
        Iterator<Particle> particleIterator = getParticles().iterator();
